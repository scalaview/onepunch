...
admin.get("/sale-report", function(req, res) {
  ...
  async.waterfall([function(next){
    sequelize.query("select count(*) AS `count`, sum(total) AS `total`, sum(cost) AS `cost`, sum(total - cost) AS `profix` FROM `ExtractOrders` AS `ExtractOrder` WHERE `ExtractOrder`.`state` = :state and `ExtractOrder`.`updatedAt` BETWEEN :begin AND :end GROUP BY DATE(updatedAt)",
